...
import (
	...
	"github.com/apache/stackinsights-swck/operator/apis/operator/v1alpha1"
)
...
const (
	...
	SidecarInjectSucceedAnno = "sidecar.stackinsights.apache.org/succeed"
	// the annotation means which container to inject
	sidecarInjectContainerAnno = "strategy.stackinsights.apache.org/inject.Container"
	// the annotation that specify the reason for injection failure
	sidecarInjectErrorAnno = "sidecar.stackinsights.apache.org/error"
	// those annotations with the following prefixes represent sidecar information
	sidecarAnnotationPrefix = "sidecar.stackinsights.apache.org/"
	// those annotations with the following prefixes represent agent information
	agentAnnotationPrefix = "agent.stackinsights.apache.org/"
	// If user want to use other Plugins' config ,the annotation must have the following form
	// plugins.stackinsights.apache.org/${config.name} = ${config.value}
	// for example , if user want to enable plugin.mongodb.trace_param
	// the annotation is plugins.stackinsights.apache.org/plugin.mongodb.trace_param: "true"
	pluginsAnnotationPrefix = "plugins.stackinsights.apache.org/"
	// If user want to use optional-plugins , the annotation must match a optinal plugin
	// such as optional.stackinsights.apache.org: "trace|webflux|cloud-gateway-2.1.x"
	// Notice , If the injected container's image don't hava the optional plugin ,
	// the container will panic
	optionsAnnotation = "optional.stackinsights.apache.org"
	// If user want to use optional-reporter-plugins , the annotation must match an optional-reporter plugin
	// such as optional-reporter.stackinsights.apache.org: "kafka"
	optionsReporterAnnotation = "optional-reporter.stackinsights.apache.org"
	// If user want to use bootstrap-plugins , the annotation must match a bootstrap plugin
	// such as bootstrap.stackinsights.apache.org: "jdk-threading"
	bootstrapAnnotation = "bootstrap.stackinsights.apache.org"
...
type SidecarInjectField struct {
	...
	// -javaagent: /sky/agent/stackinsights-agent,jar=jvmAgentConfigStr
...
// such as optional.stackinsights.apache.org: "trace|webflux|cloud-gateway-2.1.x" or
