...
type JavaSidecar struct {
	...
	// +kubebuilder:validation:Optional
	// +kubebuilder:default="inject-stackinsights-agent"
	Name string `json:"name,omitempty"`

	// Image is the image for initContainer, which commonly contains StackInsights java agent SDK.
	// +kubebuilder:validation:Optional
	// +kubebuilder:default="apache/stackinsights-java-agent:8.16.0-java8"
	Image string `json:"image,omitempty"`

	// Command is the command for initContainer.
	// +kubebuilder:validation:Optional
	// +kubebuilder:default={"mkdir -p /sky/agent && cp -r /stackinsights/agent/* /sky/agent"}
