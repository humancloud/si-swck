...
// +kubebuilder:rbac:groups=operator.stackinsights.apache.org,resources=storages,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=operator.stackinsights.apache.org,resources=storages/status,verbs=get;update;patch
...
func (r *StorageReconciler) checkSecurity(ctx context.Context, log logr.Logger, s *operatorv1alpha1.Storage) {
	...
	if tls {
		s.Spec.ServiceName = "stackinsights-storage"
	} else {
...
func (r *StorageReconciler) createCert(ctx context.Context, log logr.Logger, s *operatorv1alpha1.Storage) {
	...
	existSecret, err := clientset.CoreV1().Secrets(s.Namespace).Get(ctx, "stackinsights-storage", metav1.GetOptions{})
	if err != nil && !apierrors.IsNotFound(err) {
		log.Info("fail get stackinsights-storage secret")
		return
	}
	if existSecret.Name != "" {
	...
	subj := pkix.Name{
		CommonName:         "stackinsights-storage",
		Country:            []string{"CN"},
		Province:           []string{"ZJ"},
		Locality:           []string{"HZ"},
		Organization:       []string{"Stackinsights"},
		OrganizationalUnit: []string{"Stackinsights"},
	}
	...
	condition := certv1beta1.CertificateSigningRequestCondition{
		Type:    "Approved",
		Reason:  "ApprovedByStackinsightsStorage",
		Message: "Approved by stackinsights storage controller",
	}
	...
	p12, err := pkcs12.Encode(rand.Reader, key, cert, nil, "stackinsights")

	if err != nil {
		log.Info("fail encode pkcs12")
		return
	}
	err = clientset.CoreV1().Secrets(s.Namespace).Delete(ctx, "stackinsights-storage", metav1.DeleteOptions{})
	if err != nil && !apierrors.IsNotFound(err) {
		log.Info("fail delete secret stackinsights-storage")
		return
	}
	...
	if err != nil {
		log.Info("fail create secret stackinsights-storage")
		return
	}
	log.Info("success create secret stackinsights-storage", "secret-name", storageTLSSecret.Name)
}
